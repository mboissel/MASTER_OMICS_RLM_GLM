...
_Définition : Si aucune colonne d’une matrice n'est linéairement dépendante des autres colonnes, le rang de la matrice est égal au nombre de colonnes de la matrice et la matrice est dite de rang (colonne) plein._  
...
L’estimateur des moindres carrés ordinaires (**emco** ou “least-square method”) de $\beta$ est  
...
L’illustration suivante peut aider à comprendre la formule emco : 
...
$$L(\beta, y) = \frac{1}{(2\pi\sigma^2)^{n/2}}\mathrm{exp}\left( -\frac{||y-X\beta||^2}{2\sigma^2}\right),\ \ y \in \mathbb{R}^{n}.$$
...
On considère les hypothèses :  
H0 : $\beta_1 = \beta_2 = ...=\beta_p=0$ contre H1 : $\exists j, \beta_j \ne 0$ i.e. "il existe au moins un coefficient non nul".  
...
Vu le nuage de points à gauche, il est préférable de considérer la transformation exp(Y) et de faire une
...
## Rappel 1 – l’analyse de la variance à un facteur ou plus (ANOVA)
